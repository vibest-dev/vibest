...
export interface ChatSessionTransport extends AiChatTransport<UIMessage> {
  ...
  // Session-scoped config setters (vibest addition), driven by the model /
  // permission-mode pickers. `sessionId` is passed for symmetry; the concrete
  // transport already knows its SessionRef.
