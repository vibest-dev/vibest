...
import type { ChatModel } from "./chat-config";
import type { ChatSessionTransport } from "./chat-transport-port";

// Re-exported for consumers that historically imported the model type from the
// transport module; the canonical definition now lives in chat-config.
export type { ChatModel } from "./chat-config";
...
export class OrpcChatSessionTransport implements ChatSessionTransport {
  ...
  // `sessionId` is accepted for the port signature but redundant here: the
  // transport already carries the full SessionRef the setters key on.
  async setModel(_sessionId: string, model: string): Promise<void> {
    await this.client.session.setModel({ ref: this.#ref, model });
  }

  async setPermissionMode(_sessionId: string, permissionMode: string): Promise<void> {
