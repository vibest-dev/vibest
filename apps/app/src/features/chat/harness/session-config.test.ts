...
// The other half, listed per directory rather than declared once. Models live
...
describe("resolveModel", () => {
  ...
  it("resolves to nothing when the user picked nothing", () => {
    ...
    // the harness's own configured default — which the list cannot reveal.
    expect(resolveModel(claudeProviders, undefined, undefined)).toBeUndefined();
  });

  it("drops a pick the model list doesn't vouch for", () => {
  ...
  it("ignores a URL-supplied pick until the model list can vouch for it", () => {
...
describe("reasoningEffort cascades from the selected model", () => {
  ...
  it("resolves to nothing while the model list is still in flight", () => {
