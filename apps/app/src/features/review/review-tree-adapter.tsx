...
export function ReviewTreeAdapter({
  sessionId,
  entries,
  gitStatus,
  onSelectFile,
  ...
  gitStatus: ReadonlyArray<{
    path: string;
    status: "added" | "deleted" | "modified" | "renamed";
  }>;
  onSelectFile: (path: string) => void;
}) {
  const state = useMemo(() => getReviewFileTree(sessionId), [sessionId]);
  const containerRef = useRef<HTMLDivElement>(null);

  useLayoutEffect(() => {
    syncReviewFileTree(state, entries, gitStatus);
  }, [entries, gitStatus, state]);
  ...
  const openPath = (path: string | null): void => {
    ...
    if (!isOpenableTreeEntry(entry)) return;
    state.model.getItem(path)?.select();
    onSelectFile(path);
  };
