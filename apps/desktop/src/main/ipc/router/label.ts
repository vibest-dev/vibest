...
export const update = os.update.handler(async ({ input, context: { app } }) => {
  ...
  if (updates.name) {
    ...
    const duplicate = existing.find(
      (l) => l.id !== labelId && l.name.toLowerCase() === updates.name!.toLowerCase(),
    );
    if (duplicate) {
...
// Delete label
export const deleteLabel = os.delete.handler(async ({ input, context: { app } }) => {
  const { repositoryId, labelId, force } = input;
  const deletedFromTasks = app.store.removeLabel(repositoryId, labelId, { force });
