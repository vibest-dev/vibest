...
export const create = os.create.handler(async ({ input, context: { app } }) => {
	...
	if (
		!resolvedCwd.startsWith(worktreePath + path.sep) &&
...
// Get terminal state snapshot for tab switching
export const attach = os.attach.handler(async ({ input, context: { app } }) => {
	const { terminalId } = input;

	const snapshot = await app.terminal.getSnapshot(terminalId);
	if (!snapshot) {
		throw new Error("Terminal not found");
	}
