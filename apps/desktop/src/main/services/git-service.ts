...
export class GitService {
	...
	async getStatus(path: string): Promise<GitStatus> {
		...
		return {
			...
			modified:
				status.modified.length + status.renamed.length + status.deleted.length,
		...
		for (const { file, status: fileStatus } of filesToProcess) {
			try {
				...
				files.push({
					oldFile:
						oldContents !== null
							? { filename: file, contents: oldContents }
							: null,
					newFile:
						newContents !== null
							? { filename: file, contents: newContents }
							: null,
					status: fileStatus,
				});
			} catch {
	...
	async getDiffStats(path: string): Promise<DiffStats> {
		...
		for (const renamed of status.renamed) {
			...
			const stats = (isStaged ? stagedStats : unstagedStats).get(
				renamed.to,
			) ?? {
				insertions: 0,
				deletions: 0,
