...
export class StoreService {
	...
	updateWorktree(
		id: string,
		updates: Partial<Omit<StoredWorktree, "id">>,
	): void {
		const worktrees = this.getWorktrees();
	...
	updateTask(
		id: string,
		updates: Partial<Omit<Task, "id" | "repositoryId" | "createdAt">>,
	): void {
		const tasks = this.getTasks();
	...
	updateLabel(
		...
		const labels = (repo.labels ?? []).map((l) =>
	...
	removeLabel(
		...
		const tasksUsingLabel = this.getTasksByRepository(repositoryId).filter(
			(t) => t.labels.includes(labelId),
		);
