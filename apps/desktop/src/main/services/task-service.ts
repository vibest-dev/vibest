...
export class TaskService {
	...
	async createTask(input: {
		...
		const { path: worktreePath, placeName } =
			this.worktree.generateWorktreePath(repository.name, usedNames);
		...
		try {
			...
			try {
				await this.worktree.safeRemoveWorktree(
					repository.path,
					worktreePath,
					true,
				);
		...
		for (const worktree of worktrees) {
			if (deleteWorktree) {
				try {
					...
					console.error(
						"[TaskService] Failed to archive worktree:",
						worktree.path,
						archiveError,
					);
				}
	 ...
	 */
	getTaskWithWorktrees(
		taskId: string,
	): { task: Task; worktrees: Worktree[] } | undefined {
		const task = this.store.getTask(taskId);
	 ...
	 */
	getTasksWithWorktrees(
		repositoryId: string,
	): Array<{ task: Task; worktrees: Worktree[] }> {
