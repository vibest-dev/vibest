...
class DataBatcher {
	...
	flush(): void {
		...
		if (data.length > 0) {
			this.publisher.publish(`terminal:${this.terminalId}`, {
				type: "data",
				data,
			});
		}
...
export class TerminalManager {
	...
	create(worktreeId: string, cwd: string): TerminalInstance {
		...
		if (this.terminals.size >= TerminalManager.MAX_TERMINALS_GLOBAL) {
			throw new Error(
				`Maximum terminal limit reached (${TerminalManager.MAX_TERMINALS_GLOBAL})`,
			);
		}
		...
		if (
			worktreeTerminals.length >= TerminalManager.MAX_TERMINALS_PER_WORKTREE
		) {
			throw new Error(
		...
		// PTY data → headless (for state) + batcher (for streaming)
	...
	/**
	 * Get a snapshot of the terminal state for restoration after tab switch.
	 */
	async getSnapshot(terminalId: string): Promise<TerminalSnapshot | null> {
		...
		// Flush batcher to ensure headless terminal has all data
		instance.batcher.flush();

		// Get snapshot from headless terminal
