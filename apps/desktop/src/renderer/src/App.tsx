...
function App(): React.JSX.Element {
	...
	const [createTaskRepositoryId, setCreateTaskRepositoryId] = useState<
		string | null
	>(null);
	const [showLabelManagerDialog, setShowLabelManagerDialog] = useState(false);
	const [labelManagerRepositoryId, setLabelManagerRepositoryId] = useState<
		string | null
	>(null);
	...
	const removeRepoMutation = useMutation({
		...
		onSuccess: (_: unknown, variables: { repositoryId: string }) => {
			...
			const workspaceQueryData = queryClient.getQueryData(
				orpc.workspace.list.queryKey({}),
			);
			if (workspaceQueryData) {
				const worktrees =
					workspaceQueryData.worktreesByRepository[variables.repositoryId];
	...
	const selectedRepository = useMemo(() => {
		if (selectedTask) {
			return (
				repositories.find((r) => r.id === selectedTask.repositoryId) ?? null
			);
		}
		if (selectedWorktree) {
			return (
	...
	return (
		<div className="bg-background text-foreground flex h-screen overflow-hidden">
			...
			<CreateTaskDialog
				...
				repository={
					createTaskRepositoryId
						? (repositories.find((r) => r.id === createTaskRepositoryId) ??
							null)
						: null
				}
				onClose={() => {
			...
			<EditTaskDialog
				...
				repository={
					editTaskTarget
						? (repositories.find((r) => r.id === editTaskTarget.repositoryId) ??
							null)
						: null
				}
				onClose={() => {
			...
			<LabelManagerDialog
				...
				repository={
					labelManagerRepositoryId
						? (repositories.find((r) => r.id === labelManagerRepositoryId) ??
							null)
						: null
				}
				onClose={() => {
