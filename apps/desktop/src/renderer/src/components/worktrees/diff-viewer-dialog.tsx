...
export function DiffViewerDialog({ isOpen, worktree, onClose }: DiffViewerDialogProps) {
  if (!worktree) return null;

  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      {/* The popup content only mounts while the dialog is open, and `key` remounts it
          for a different worktree, so the diff is fetched fresh and the selection starts
          at the first file on every open — no reset effect needed. */}
      <DialogPopup className="h-[80vh] sm:max-w-5xl">
        <DiffViewerContent key={worktree.path} worktree={worktree} />
      </DialogPopup>
    </Dialog>
  );
}

function DiffViewerContent({ worktree }: { worktree: Worktree }) {
  const [selectedFileIndex, setSelectedFileIndex] = useState<number>(0);
  const [showStaged, setShowStaged] = useState(false);

  // useDiff refetches whenever `path` or `staged` changes, and on mount
