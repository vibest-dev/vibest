...
export function removeRepositoryMutationCallbacks() {
	return {
		onSuccess: (_: unknown, variables: { repositoryId: string }) => {
			...
			const workspaceData = queryClient.getQueryData(
				orpc.workspace.list.queryKey({}),
			);
			if (workspaceData) {
				const worktrees =
					workspaceData.worktreesByRepository[variables.repositoryId];
...
export function removeWorktreeMutationCallbacks() {
	return {
		onSuccess: (
			_: unknown,
			variables: { worktreeId: string; force?: boolean },
		) => {
			const state = uiStore.getState();
...
export function archiveWorktreeMutationCallbacks() {
	return {
		onSuccess: (
			_: unknown,
			variables: { worktreeId: string; commitFirst?: boolean },
		) => {
			const state = uiStore.getState();
			if (state.selectedWorktreeId === variables.worktreeId) {
				state.selectWorktree(null);
