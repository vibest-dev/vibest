...
onMessage(VibestExtensionMessage.WebAppInit, async (event) => {
  return await sendMessage(VibestExtensionMessage.WebAppInit, event.data, "sidepanel");
});
onMessage(VibestExtensionMessage.Inspected, async (event) => {
  return await sendMessage(VibestExtensionMessage.Inspected, event.data, "sidepanel");
});
