...
- `SessionService`：projectId→cwd 解析（经 ProjectService）、生成 daemon uuid sessionId、写元数据、**daemon SessionRef ↔ harnessSessionId 翻译**（`resolveHarnessSessionId` 供后续 prompt/interrupt/subscribe 复用）、create/resume/close/list；create 失败清理（元数据写失败则关闭 native 不泄漏）。9 服务测试（fake port）覆盖四类错误路径。
