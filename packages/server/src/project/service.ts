...
/**
 * `project` module: list / create (path-dedup) / remove (playground-protected)
 * / findById. Business rules live here; persistence is delegated to the repo.
 */
export class ProjectService extends Context.Service<
  ProjectService,
  {
    ...
    readonly remove: (
      id: string,
    ) => Effect.Effect<void, StoreReadError | StoreWriteError | ProjectNotFound | ProjectProtected>;
  }
>()("ProjectService") {}
        ...
        remove: (id) =>
          Effect.gen(function* () {
            ...
            if (target.protected === true) {
              return yield* Effect.fail(new ProjectProtected({ projectId: id }));
            }
