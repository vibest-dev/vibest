...
/** Session-scoped config the server may seed at create time (vibest addition). */
...
export class HarnessAgentSessionPort extends Context.Service<
  HarnessAgentSessionPort,
  {
    ...
    // Session-scoped config setters (vibest addition). Harnesses that don't
    // support a knob accept the call and no-op.
    readonly setModel: (
      harnessSessionId: string,
      model: string,
  ...
  Effect.gen(function* () {
    ...
    return {
      create: (harnessAgentId, workspacePath, config) =>
        harness.create(harnessAgentId, { workspacePath, ...config }).pipe(
      ...
      setPermissionMode: (harnessSessionId, mode) =>
        harness.setPermissionMode(harnessSessionId, mode),
    };
  }),
);
