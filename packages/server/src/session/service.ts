...
export class SessionService extends Context.Service<
  SessionService,
  {
    ...
    >;
    /** Change the live session's model (vibest addition). */
    readonly setModel: (
      ref: SessionRef,
      model: string,
    ) => Effect.Effect<
    ...
    >;
    /** Change the live session's permission mode (vibest addition). */
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      setPermissionMode: (ref, mode) =>
        resolveHarnessSessionId(ref).pipe(
          Effect.flatMap((id) => port.setPermissionMode(id, mode)),
        ),
