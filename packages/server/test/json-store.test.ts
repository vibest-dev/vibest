...
import { readJson, readJsonDir, removeFile, writeJsonAtomic } from "../src/infra/json-store";
...
describe("removeFile", () => {
  it("fails with StoreWriteError when the delete is refused", async () => {
    const error = await run(
      Effect.flip(removeFile("/store/sessions/s1.json")),
      fakeFileSystem({ remove: (path) => Effect.fail(permissionDenied("remove", path)) }),
    );
    expect(error).toBeInstanceOf(StoreWriteError);
    expect(error.file).toBe("/store/sessions/s1.json");
  });
});

describe("readJsonDir", () => {
  const withPath = (fs: Layer.Layer<FileSystem.FileSystem>) =>
    Layer.provideMerge(fs, NodePlatformLayer);

  it("yields [] for a directory that was never written", async () => {
    const value = await Effect.runPromise(
      Effect.provide(readJsonDir("/store/sessions"), withPath(fakeFileSystem({}))),
    );
    expect(value).toEqual([]);
  });

  it("fails with StoreReadError when one entry is malformed", async () => {
    const error = await Effect.runPromise(
      Effect.provide(
        Effect.flip(readJsonDir("/store/sessions")),
        withPath(
          fakeFileSystem({
            readDirectory: () => Effect.succeed(["ok.json", "broken.json", "notes.txt"]),
            readFileString: (path) =>
              String(path).endsWith("broken.json")
                ? Effect.succeed("{ not json")
                : Effect.succeed(`{"id":"s1"}`),
          }),
        ),
      ),
    );
    expect(error).toBeInstanceOf(StoreReadError);
    expect(error.file).toBe("/store/sessions");
  });

  it("propagates a read failure that is not NotFound", async () => {
    const error = await Effect.runPromise(
      Effect.provide(
        Effect.flip(readJsonDir("/store/sessions")),
        withPath(
          fakeFileSystem({
            readDirectory: (path) => Effect.fail(permissionDenied("readDirectory", path)),
          }),
        ),
      ),
    );
