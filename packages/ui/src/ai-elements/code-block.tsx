...
async function highlight(code: string, options: CodeToHastOptions): Promise<JSX.Element> {
    ...
    <CodeBlockContext.Provider value={{ code }}>
      ...
      >
        <Suspense fallback={<p className="p-2 text-xs">Loading...</p>}>
          <div className="scrollbar-hide overflow-auto px-4 py-3.5 font-mono text-sm">
            <HighlightedCode promises={highlightPromise} />
          </div>
        </Suspense>
        {children && (
          <div className="absolute top-1 right-2 flex items-center gap-2">{children}</div>
        )}
      </div>
