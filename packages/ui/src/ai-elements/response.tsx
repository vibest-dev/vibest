...
const MemoA = memo<AProps>(
	({ children, className, href, node, ...props }: AProps) => (
		<a
			className={cn(
				"wrap-anywhere font-medium text-primary underline text-sm",
				className,
			)}
			href={href}
			rel="noreferrer"
...
const MemoH1 = memo<HeadingProps<"h1">>(
	({ children, className, node, ...props }) => (
		<h1
			className={cn("mt-5 mb-1 font-semibold text-2xl", className)}
			{...props}
		>
			{children}
		</h1>
...
const MemoH2 = memo<HeadingProps<"h2">>(
	({ children, className, node, ...props }) => (
		<h2 className={cn("mt-5 mb-1 font-semibold text-xl", className)} {...props}>
			{children}
		</h2>
	),
	(p, n) => sameClassAndNode(p, n),
...
const MemoH5 = memo<HeadingProps<"h5">>(
	({ children, className, node, ...props }) => (
		<h5 className={cn("mt-3 mb-1 font-semibold text-sm", className)} {...props}>
			{children}
		</h5>
	),
	(p, n) => sameClassAndNode(p, n),
...
const MemoH6 = memo<HeadingProps<"h6">>(
	({ children, className, node, ...props }) => (
		<h6 className={cn("mt-3 mb-1 font-semibold text-xs", className)} {...props}>
			{children}
		</h6>
	),
	(p, n) => sameClassAndNode(p, n),
...
const MemoBlockquote = memo<BlockquoteProps>(
	({ children, className, node, ...props }: BlockquoteProps) => (
		<blockquote
			className={cn(
				"my-4 border-muted-foreground/30 border-l-4 pl-4 text-muted-foreground italic text-sm",
				className,
			)}
			{...props}
		>
	...
	({ children, className, node, ...props }: SectionProps) => {
		...
		if (isFootnotesSection) {
			const isEmptyFootnote = (listItem: ReactNode): boolean => {
				...
				const listElement = listItem as ElementWithProps<
				...
				for (const itemChild of itemChildren) {
					...
					if (typeof itemChild === "string") {
						...
						const childProps = itemChild.props as ElementProps<
						...
						if (childRecord["data-footnote-backref"] !== undefined) {
							...
							for (const grandChild of grandChildren) {
								...
								if (isValidElement(grandChild)) {
									const grandChildProps = grandChild.props as ElementProps<
										Record<string, unknown>
									>;
									const grandChildRecord = grandChildProps as Record<
										string,
										unknown
									>;
			...
			const processedChildren = Array.isArray(children)
				? children.map((child) => {
						...
						if (child.type === MemoOl) {
							const orderedList = child as ElementWithProps<
...
const CodeComponent = ({
	...
	return (
		<CodeBlock
			className={cn(
				"overflow-x-auto border-t [&+p]:mt-2 [p+&]:mt-2",
				className,
			)}
			code={code}
