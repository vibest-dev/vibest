...
export function VibestClientRpcProvider({ children }: { children?: React.ReactNode }) {
  const context = useVibestClientRpc();

  return (
    <VibestClientRpcContext.Provider value={context}>{children}</VibestClientRpcContext.Provider>
  );
}
...
export const useVibestClientRpcContext = () => {
  ...
  if (!context) {
    throw new Error("useVibestClientRpcContext must be used within a VibestClientRpcProvider");
  }
  return context;
};
